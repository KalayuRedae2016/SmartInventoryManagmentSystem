<template>
  <div class="space-y-4">
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-bold text-brand">Categories</h2>
...
async function save(formData) {
  ...
  try {
    ...
    alert(error?.response?.data?.message || error?.message || 'Unable to save category')
    return
  }
}
...
async function toggleStatus(row) {
  try {
    await store.toggleCategoryStatus(row)
  } catch (error) {
    alert(error?.response?.data?.message || error?.message || 'Unable to update category status')
  }
}
