<template>
  <div class="space-y-4">
    <div class="flex justify-between items-center">
      <h2 class="text-2xl font-bold text-brand">Brands</h2>
...
const confirmTitle = computed(() => {
  const name = rowToDelete.value?.name ? `"${rowToDelete.value.name}"` : 'this brand'
  return `Delete ${name}?`
})
...
async function save(formData) {
  ...
  try {
    ...
    alert(error?.response?.data?.message || error?.message || 'Unable to save brand')
    return
  }
}
...
async function confirmDelete() {
  ...
  try {
    await store.deleteBrand(rowToDelete.value.id)
  } catch (error) {
    alert(error?.response?.data?.message || error?.message || 'Unable to delete brand')
  } finally {
...
async function toggleStatus(row) {
  try {
    await store.toggleBrandStatus(row)
  } catch (error) {
    alert(error?.response?.data?.message || error?.message || 'Unable to update brand status')
  }
}
